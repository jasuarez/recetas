(function() {
  function displaySearchResults(results, store) {
    ...
    if (results.length) { // Are there any results?
	  ...
	  if (item.menu == "primero")
            appendString += '<li><a href="' + item.url + '">' + item.title + '</a>';
        }
        searchResults.innerHTML += appendString.length > 0 ? '<h2>primeros</h2>' + appendString : '';
      
        var appendString = '';
        for (var i = 0; i < sortResults.length; i++) {  // Iterate over the results
          var item = sortResults[i];
	  if (item.menu == "segundo")
            appendString += '<li><a href="' + item.url + '">' + item.title + '</a>';
        }
        searchResults.innerHTML += appendString.length > 0 ? '<h2>segundos</h2>' + appendString : '';
	  ...
	  if (item.menu == "postre")
        ...
        searchResults.innerHTML += appendString.length > 0 ? '<h2>postres</h2>' + appendString : '';
	  ...
	  if (item.menu == "otro")
        ...
        searchResults.innerHTML += appendString.length > 0 ? '<h2>otros</h2>' + appendString : '';
